...
def train_episode(env, agent):
    ...
    while True:
        ...
        agent.learn(state, action, reward, next_state, done)     #只用terminated试一试
        total_reward += reward
        state = next_state
        action = next_action
        if done:
...
def train(env, episodes=1000, lr=0.1, gamma=0.9, epsilon=0.1):
...
if __name__ == "__main__":
    train_env = gym.make("CliffWalking-v0", render_mode="None", max_episode_steps=500)
    test_env = gym.make("CliffWalking-v0", render_mode="human", max_episode_steps=500)
